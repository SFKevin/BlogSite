...
<body>
	<!-- 顶部 -->
	<jsp:include page="base/Androidtopper.jsp" />
	<!-- 中间内容 -->
	<div class="home">
			...
			<div class="content fl">
				<div class="blog-tabs">
					...
					<c:if test="${pageNo gt 1}">
						<a href="${path}/androidAll.action?pageNo=1">首页</a>
					</c:if>
					<c:if test="${pageNo eq 1}">上一页</c:if>
					<c:if test="${pageNo gt 1}">
						<a href="${path}/androidAll.action?pageNo=${pageNo-1}">上一页</a>
					</c:if>
					<c:if test="${pageNo eq maxPage}">下一页</c:if>
					<c:if test="${pageNo lt maxPage}">
						<a href="${path}/androidAll.action?pageNo=${pageNo+1}">下一页</a>
					</c:if>
					<c:if test="${pageNo eq maxPage}">末页</c:if>
					<c:if test="${pageNo lt maxPage}">
						<a href="${path}/androidAll.action?pageNo=${maxPage}">末页</a>
					</c:if>
				</div>
			</div>
