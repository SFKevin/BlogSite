			...
			<div class="content fl">
				<div class="blog-tabs">
					...
					<c:if test="${pageNo gt 1}">
						<a href="${path}/javaAll.action?pageNo=1">首页</a>
					</c:if>
					<c:if test="${pageNo eq 1}">上一页</c:if>
					<c:if test="${pageNo gt 1}">
						<a href="${path}/javaAll.action?pageNo=${pageNo-1}">上一页</a>
					</c:if>
					<c:if test="${pageNo eq maxPage}">下一页</c:if>
					<c:if test="${pageNo lt maxPage}">
						<a href="${path}/javaAll.action?pageNo=${pageNo+1}">下一页</a>
					</c:if>
					<c:if test="${pageNo eq maxPage}">末页</c:if>
					<c:if test="${pageNo lt maxPage}">
						<a href="${path}/javaAll.action?pageNo=${maxPage}">末页</a>
					</c:if>
				</div>
			</div>
